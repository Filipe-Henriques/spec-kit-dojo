...
- **No recorded failure observation.** "I wrote a test for it" is insufficient — the agent MUST capture the failing-run output to a structured evidence file under `tests/unit/.red-evidence/` (path-encoded by test file; see `/speckit.sybilion-dojo.validate` Pre-flight #7 for the exact naming convention and block format) before writing the implementation that turns the test green. The evidence MUST include the exact command run, an ISO 8601 timestamp, the non-zero exit code, and the captured stderr/stdout (assertion message + traceback). One block per test function. The `.red-evidence/` directory is testing evidence, not build output — it MUST be committed to the repository. Forging this file from memory is a constitutional violation — the captured output contains real line numbers and runner-specific framing that cannot be plausibly reconstructed. The Sybilion-Dojo `validate` command enforces this artifact as a pre-flight gate. When a Claude Code plug-in provides equivalent enforcement (e.g. Superpowers' `test-driven-development` skill), the file-based capture is satisfied by that skill's own evidence.
